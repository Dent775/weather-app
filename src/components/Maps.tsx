...
export default function Maps({coords,onMapClick,mapType}: Props) {
  ...
  return (
    <MapContainer center={[lat, lon]} zoom={5} scrollWheelZoom={true} style={{width:"vw",height:"500px"}}>
      <MapClick onMapClick={onMapClick} coords={coords}/>
  <MapTileLayer/>
  {mapType!=="Default"&&<TileLayer url={`https://tile.openweathermap.org/map/${mapType}/{z}/{x}/{y}.png?appid=${API_KEY}`}/>}

  {mapType==="Satellite"&&<TileLayer url='https://api.maptiler.com/maps/hybrid-v4/{z}/{x}/{y}.jpg?key=ygy36deHjX805SM4GoUP'/>}
